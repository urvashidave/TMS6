...
$series1 = array();
$series1['name'] = 'Avg G2G <br>From: '.$from.'-'.$month.'-'.$year.'<br> To '.$to.'-'.$month.'-'.$year;

$query = $mysqli->query("select folio_no,exit_time,entry_time,load_start_time,load_stop_time from TransHeader where (folio_no BETWEEN $from AND $to) AND (folio_mo = $month) AND (folio_yr = $year) group by folio_no");
//echo ("select folio_no,exit_time,entry_time,load_start_time,load_stop_time from TransHeader where (folio_no BETWEEN $from AND $to) AND (folio_mo = $month) AND (folio_yr = $year) group by folio_no");
